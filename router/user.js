...
router.post('/upload',function(req,res){
  ...
  // 设置临时保存路径
  form.uploadDir = './temp'; // 在请求里面的路径要看启动文件所在位置寻找,在页面要根据设置根目录去查找
  // 解析请求对象
  form.parse(req,function(err,fields,files){
      if(err){
        console.log(err);
        res.render('error',{errMsg:"上传送头像失败"});
        return;
      }
      // 获取图片对象
      var pic = files.pic;
...
router.get('/cut',function(req,res){
  ...
  gm('./temp'+img).crop(w,h,x,y).write(avatar,function(err){
    ...
    // 剪切成功,将新头像的路径保存到数据库
    db.modify(User,{username:username},{avatar:username+i},function(err,raw){
      if(err){
        console.log(err);
        res.send({status:1,msg:"修改失败"});
        return;
